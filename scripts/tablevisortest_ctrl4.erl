...
scenario(flow_stats_requests) ->
  multiply_msg(flow_stats_request,1000);
    ...
    receive
        {connected, Type, Socket, Pid} ->
            ...
            [begin
                 ...
                 Ts = generation_id(),
                 lager:info("~p: send    ~p",[Ts, Xid]),
                 file:write_file("/home/sherrnleben/erl.log", io_lib:fwrite("~p: send    ~p\n",[Ts, Xid]), [append]),
...
%%             end;
        {msg, Socket, Message} ->
          ...
          Xid = Message#ofp_message.xid,
          Ts = generation_id(),
          lager:info("~p: receive ~p",[Ts, Xid]),
          file:write_file("/home/sherrnleben/erl.log", io_lib:fwrite("~p: receive ~p\n",[Ts, Xid]), [append]),
          % tablevisor evaluation end
